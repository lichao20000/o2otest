...
class Setting extends React.Component{
        ...
        axios({
            ...
            if(resp.status == 200){
                 if(resp.data instanceof Array){
                     this.setState({channels: resp.data})
                 }else{
    ...
    confirm(){
        ...
        axios({
            url: '/user/api/set_sales_info.json' ,
            transformRequest:[ function(data, headers) {
                let _data =  []
                for(let k in data){
                    _data.push(k+'='+ data[k])
    ...
    render(){
        ...
        return (
            <div> 
                <MuiThemeProvider >
                    <Paper className='setting-paper text-center' zDepth={1}>
                        <h3> 设置渠道和区分信息</h3>
                        {
...
ReactDOM.render(<Setting/>, document.getElementById('divSetting'))
